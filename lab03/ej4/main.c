...
int main(int argc, char *argv[]) {
    ...
    int maxTemps[YEARS];
    month_t mesesMaxPrec[YEARS];

    printf("La menor temperatura minima registrada en la ciudad de Cordoba es %d\n",minTemPH(array));
    (procedimiento(array,maxTemps));
    (procedimiento2(array,mesesMaxPrec));

    for(int a=0; a<YEARS;++a){
        printf("La mayor temperatura maxima de %d fue %d\n", a+FST_YEAR, maxTemps[a]);
    };
    for(int a=0; a<YEARS;++a){
        printf("La mayor cantidad de precipitaciones de %d fue en el mes %d\n", a+FST_YEAR, mesesMaxPrec[a]+1);
    };     
